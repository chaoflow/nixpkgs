...
my $venv = "./env";
my $vpython = "$venv/bin/python";
my $vsite = "$venv/lib/$python/site-packages";

subtest "virtualenv: initialises $python", sub {
    $machine->succeed("virtualenv -p $python $venv");
    $machine->succeed("$vpython -c 'import sys; sys.exit({True: 0, False: 1}[\"python\" + sys.version[:3] == \"$python\"])'");
    #$machine->succeed("$vpython -c 'import sys,pprint;pprint.pprint(sys.path)'");
};


# XXX: Add tests for all python.modules, see profile.pl


subtest "$python virtualenv: isolated from profile", sub {
    $machine->fail("$vpython -c 'import flake8'");
};


# connect virtualenv to profil
subtest "$python virtualenv: make profile site visible to virtualenv", sub {
    print $machine->succeed("ls -l /run/current-system/sw/lib/python2.7");
    print $machine->succeed("ls -l /run/current-system/sw/lib/python2.7/site-packages");
    $machine->succeed("realpath --no-symlinks --relative-to $vsite $site > $vsite/nixprofile.pth");
    $machine->succeed("$vpython -c 'import flake8'");
    #$machine->succeed("./$venv/bin/easy_install -H \"\" flake8");
};
